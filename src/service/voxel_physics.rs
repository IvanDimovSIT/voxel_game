use macroquad::{camera::{set_camera, Camera3D}, color::WHITE, math::{vec3, Vec3}, models::{draw_cube, Mesh}, texture::Texture2D};

use crate::{graphics::{falling_shader::FallingShader, renderer::Renderer}, model::{area::AREA_HEIGHT, location::Location, voxel::Voxel, world::World}, utils::{vector_to_location, StackVec}};

use super::camera_controller::CameraController;

const FALLING_VOXELS: [Voxel; 1] = [Voxel::Sand];
...
impl VoxelSimulator {
    pub fn new() -> Self {
        Self { simulated_voxels: vec![], shader: FallingShader::new() }
    }
    ...
    pub fn update_voxels(&mut self, world: &mut World, renderer: &mut Renderer, location_to_check: Location) {
        ...
        for location in to_check {
            ...
            // recursive call
            let up_location = Location { z: location.z - 1, ..location };
    ...
    fn draw_voxel(simulated_voxel: &SimulatedVoxel) {
        draw_cube(simulated_voxel.position, Vec3::ONE, Some(&simulated_voxel.texture), WHITE);
    }
}