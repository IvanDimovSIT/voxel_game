...
use crate::{
    model::{
        area::{AREA_SIZE, Area, AreaLocation},
...
impl HeightMap {
    pub fn new() -> Self {
        Self {
            pixel_buffer: Image::gen_image_color(IMAGE_SIZE, IMAGE_SIZE, WHITE),
    ...
    pub fn generate_height_map(
        ...
        for visible_area_location in visible_area_locations {
            let max_height = world
                .get_area_without_loading(visible_area_location)
                .get_max_height();
            let area_offset_x =
        ...
        let height_map = Texture2D::from_image(&self.pixel_buffer);
        match user_settings.shadow_type {
            ShadowType::Soft => height_map.set_filter(macroquad::texture::FilterMode::Linear),
            ShadowType::Hard => height_map.set_filter(macroquad::texture::FilterMode::Nearest),
